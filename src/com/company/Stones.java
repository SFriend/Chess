	...
	public Stones() {
		for (int x1 = 0; x1 < 8; x1++) {
			for (int y1 = 0; y1 < 8; y1++) {
				...
				if (brd.getField(temp_point).isColor(0))
					gBuffer.setColor(col[countColor[2] % col.length]);
				else if (brd.getField(temp_point).isColor(1))
...
//				gBuffer.setFont(new Font("Arial", Font.PLAIN, 20*width/100));
				...
				char temp = figuren.charAt(brd.getField(temp_point).getPiece());
...
//			pawn(x-f.getX(), y-f.getY()); // TODO Pointer
	...
	public void LastMove(int x, int y){
		...
		y *= width;
		int secColor = 0;
		while(checkForSameColorSelection(secColor)){ secColor++; }  //TODO: integrieren
	...
	public void ChoseStone(int x, int y){
		for(int i = 0; i < 4; i++){
			...
			if (brd.getField(temp_point).isColor(0)) {
				gBuffer.setColor(col[countColor[2] % col.length]);
			} else if (brd.getField(temp_point).isColor(6)) {
				gBuffer.setColor(col[countColor[3] % col.length]);
			}
